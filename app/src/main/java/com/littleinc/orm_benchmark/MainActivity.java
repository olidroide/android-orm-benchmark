...
public class MainActivity extends FragmentActivity {
  ...
  public void runBenchmark(View v) {
    if (mCount < NUM_ITERATIONS) {
      ...
      new ProfilerTask(v).execute(CREATE_DB, WRITE_DATA, READ_DATA, READ_INDEXED, READ_SEARCH,
          DROP_DB);
    } else {
  ...
  private String buildResults() {
    ...
    for (Task task : Task.values()) {
      ...
      for (BenchmarkExecutable orm : mOrms) {
        ...
        Map<Task, List<Long>> results = mGlobalResults.get(orm.getProfilerId());
        if (results == null) {
          continue orms;
        }
        ...
        builder.append(orm.getOrmName())
            .append(" - Avg: ")
            .append(Util.formatElapsedTime(resultsCount / numExecutions))
            .append("<br />");
      }
      builder.append("<br />");
    }
  ...
  private class ProfilerTask extends AsyncTask<Task, Void, Void> {
    ...
    private void addProfilerResult(int profilerId, Task task, long result) {
      Map<Task, List<Long>> profilerResults = mGlobalResults.get(profilerId);
